 ...
 *
 * @author tag
 * @version $Id: HighResolutionTerrain.java 14177 2010-12-03 00:28:05Z tgaskins $
 */
public class HighResolutionTerrain extends WWObjectImpl implements Terrain
{
    /** Holds a tile's geometry. It's heavyweigh so cached when created to enable re-use. */
...
//            System.out.println(2 * 4 + 4 + this.vertices.length * 3 * 4);
    ...
    // anew for each intersection or getSurfacPoint operation.
...
//    }
    ...
    protected RectTile createTile(int row, int col)
    {
        ...
        double minLon = this.sector.getMinLongitude().degrees + col * this.lonTileSize;
        double maxLon = minLon + this.lonTileSize;

        double minLat = this.sector.getMinLatitude().degrees + row * this.latTileSize;
        double maxLat = minLat + this.latTileSize;
