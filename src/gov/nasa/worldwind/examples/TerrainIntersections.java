 ...
 * <p/>
 * If the highest resolution terrain is not availble for the area around the selected location, it is retrieved from the
 * elevation model's source, which is most likely a remote server. Since the high-res data must be retrieved and then
 * loaded from the local disk cache, it will take some time to compute and show the intersections.
 * <p/>
...
public class TerrainIntersections extends ApplicationTemplate
{
    ...
    {
        private static final Cursor WaitCursoer = new Cursor(Cursor.WAIT_CURSOR);
...
//
//        protected RenderableLayer tileGeometryLayer;
//        protected List<FloatBuffer> vertexLists;// = new ArrayList<FloatBuffer>();
//        protected List<Vec4> refCenters;// = new ArrayList<Vec4>();
...
//
//            this.tileGeometryLayer = new TileGeometryLayer();
//            this.tileGeometryLayer.setName("Tile Geometry");
//            this.tileGeometryLayer.setEnabled(false);
//            this.tileGeometryLayer.setPickEnabled(false);
//            this.getWwd().getModel().getLayers().add(this.tileGeometryLayer);
//
//            this.tilesLayer = new RenderableLayer();
//            this.tilesLayer.setName("Intersecting Tiles");
//            this.tilesLayer.setEnabled(false);
//            this.getWwd().getModel().getLayers().add(this.tilesLayer);
...
//            if (this.vertexLists != null)
//            {
//                this.vertexLists.clear();
//                this.refCenters.clear();
//            }
...
//
//                if (this.vertexLists != null)
//                    this.getGeometry(centerPosition, gridPos);
...
//
//        protected void getGeometry(Position pA, Position pB) throws InterruptedException
//        {
//            this.indices = this.terrain.getIntersectingTilesGeometry(pA, pB, this.vertexLists, this.refCenters);
//        }
...
//
//        protected void showIntersectingTiles(List<Position> grid, Position cPos)
//        {
//            this.tilesLayer.removeAllRenderables();
//
//            // Display intersecting tiles from the center to each grid point.
//            ShapeAttributes lineAttributes;
//            lineAttributes = new BasicShapeAttributes();
//            lineAttributes.setDrawOutline(true);
//            lineAttributes.setDrawInterior(false);
//            lineAttributes.setOutlineMaterial(Material.MAGENTA);
//            lineAttributes.setOutlineOpacity(1);
//
//            for (Position p : grid)
//            {
//                List<Sector> sectors = this.terrain.getIntersectionTiles(cPos, p);
//                if (sectors == null)
//                    continue;
//
//                for (Sector sector : sectors)
//                {
//                    sector = this.scaleSector(sector, 0.9);
//                    LatLon[] corners = sector.getCorners();
//                    List<Position> endPoints = new ArrayList<Position>(5);
//                    for (LatLon corner : corners)
//                    {
//                        endPoints.add(new Position(corner.getLatitude(), corner.getLongitude(), 0));
//                    }
//                    endPoints.add(new Position(corners[0].getLatitude(), corners[0].getLongitude(), 0));
//
//                    Path path = new Path(endPoints);
//                    path.setAltitudeMode(WorldWind.CLAMP_TO_GROUND);
//                    path.setAttributes(lineAttributes);
//                    this.tilesLayer.addRenderable(path);
//                }
//            }
//        }
//
//        protected Sector scaleSector(Sector sector, double scale)
//        {
//            double cLat = sector.getCentroid().getLatitude().degrees;
//            double cLon = sector.getCentroid().getLongitude().degrees;
//            double dLat = sector.getDeltaLatDegrees();
//            double dLon = sector.getDeltaLonDegrees();
//
//            double minLat = cLat - 0.5 * scale * dLat;
//            double maxLat = cLat + 0.5 * scale * dLat;
//            double minLon = cLon - 0.5 * scale * dLon;
//            double maxLon = cLon + 0.5 * scale * dLon;
//
//            return Sector.fromDegrees(minLat, maxLat, minLon, maxLon);
//        }
//
//        class TileGeometryLayer extends RenderableLayer
//        {
//            @Override
...
//                    {
//                        dc.getView().pushReferenceCenter(dc, refCenters.get(i));
//                        try
//                        {
//                            gl.glVertexPointer(3, GL.GL_FLOAT, 0, vertexLists.get(i).rewind());
//                            gl.glDrawElements(GL.GL_TRIANGLE_STRIP, indices.limit(), GL.GL_UNSIGNED_INT,
//                                indices.rewind());
//                        }
//                        finally
//                        {
