 ...
 * specified, a filled rectangle will be drawn in its place.
 *
 * @author tag
 * @version $Id: ScreenImage.java 13943 2010-10-06 20:10:19Z dcollins $
 */
public class ScreenImage implements Renderable, Exportable
{
    ...
    /**
     * Convenience method to specify the location of the image on the screen. The position is relative to the upper-left
     * corner of the World Window. The image is centered on this position.
     *
    ...
    {
        // Use units PIXELS for the X and Y screen offset; the image's location is interpreted as an offset in pixels
    ...
    /**
     * Set the offset of the image relative to viewport. The screen point identified by this offset will be aligned to
     * the image point identified by the image offset.
     *
    ...
    protected void computeOffsets(DrawContext dc)
    {
        if (dc.getFrameTimeStamp() != this.frameNumber)
        {
            ...
            {
                // Don't translate from AWT to OpenGL coordinates here During rendering ScreenImage applies a transform
                // that translates from AWT to OpenGL coordinates, and therefore interprets the screen location as an
    ...
    protected void draw(DrawContext dc)
    {
        ...
        try
        {
            ...
            // Apply the screen location transform.
            this.applyScreenLocationTransform(dc);
     ...
     * system has its origin in the upper left corner, with the X axis pointing right and the Y axis pointing down.
     *
     * @param dc the DrawContext the transform is applied to.
     */
    protected void applyScreenLocationTransform(DrawContext dc)
    {
        // Translate from AWT to OpenGL coordinates by inverting the Y axis, then add the image offset. The dx and dy
        // parameters are the negative of the image offset X and Y. Therefore we add dx and dy to the screen location
        // instead of subtracted them.
        Rectangle viewport = dc.getView().getViewport();
        dc.getGL().glTranslated(this.screenLocation.x + this.dx, viewport.height - this.screenLocation.y + this.dy, 0d);
    }
