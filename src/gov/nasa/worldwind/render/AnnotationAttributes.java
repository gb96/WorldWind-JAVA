 ...
 * @version $Id: AnnotationAttributes.java 13909 2010-09-30 06:33:58Z pabercrombie $
...
public class AnnotationAttributes implements Restorable
{
     ...
     * smaller or larger then the preferred size depending on the text. Can be one of {@link AVKey#SIZE_FIXED}
     * (default) or {@link AVKey#SIZE_FIT_TEXT}. Setting this attribute to <code>SIZE_FIT_TEXT</code> would have
     * the callout drawn at its exact width (see setSize()).
     *
    ...
    {
        // Map legacy versions of the Annotation constants to the new AVKey constants
        ...
        {
            // Map legacy versions of Size constants to new values
            String upgradedValue = legacySupport.get(adjustWidthToTextState);
            if (upgradedValue != null)
                adjustWidthToTextState = upgradedValue;
        ...
        {
            // Map legacy versions of Repeat constants to new values
            String upgradedValue = legacySupport.get(imageRepeatState);
            if (upgradedValue != null)
                imageRepeatState = upgradedValue;
