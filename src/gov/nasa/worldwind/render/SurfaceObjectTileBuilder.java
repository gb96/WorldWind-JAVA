 ...
 *
 * @author dcollins
 * @version $Id: SurfaceObjectTileBuilder.java 14197 2010-12-07 00:21:43Z dcollins $
 */
public class SurfaceObjectTileBuilder
{
    ...
    /** The unique cache name. Associates the tile builder with it's cached tiles. */
    ...
    /**
     * Returns a shared {@link gov.nasa.worldwind.util.LevelSet} who's tile dimension is a power of two that fits in the
     * specified DrawContext's viewport. The returned LevelSet's tile dimension is always square and a power of two. If
     * the viewport is larger than the tile builder's {@link #tileWidth} and {@link #tileHeight}, this returns a
     * LevelSet with tile dimension set to the maximum of <code>tileWidth</code> and <code>tileHeight</code>. Otherwise,
     * this returns a LevelSet with tile dimension set to the smallest power of two that fits in the viewport.
     * <p/>
     * The returned LevelSet's cache name and dataset name are dummy values, and should not be used. Use the tile
     * builder's cache name instead.
     * <p/>
    ...
    protected LevelSet getLevelSet(DrawContext dc)
    {
        ...
        if (levelSet == null)
        {
            levelSet = createLevelSet(dimension.width, dimension.height);
     ...
     * resolution criteria. The tiles belong to the LevelSet returned by {@link #getLevelSet(DrawContext)}, and are
     * added to the {@link #currentTiles} list. Tiles are culled against the current SurfaceObject list, against the
     * DrawContext's view frustum during rendering mode, and against the DrawContext's pick frustums during picking
     * mode. If a tile does not meet the tile builder's resolution criteria, it's split into four sub-tiles and the
     * process recursively repeated on the sub-tiles.
     * <p/>
            ...
            for (Sector s : sectors)
            {
                ...
                for (int row = firstRow; row <= lastRow; row++)
                {
                    ...
                    for (int col = firstCol; col <= lastCol; col++)
                    {
                        ...
                        Object tileKey = this.createTileKey(level, row, col);
                        SurfaceObjectTile tile = (SurfaceObjectTile) TextureTile.getMemoryCache().getObject(tileKey);
                        if (tile == null)
                        {
                            tile = this.createTile(new Sector(p1, p2, t1, t2), level, row, col);
     ...
     * and column}. The returned tile's cache name is set to {@link #cacheName}.
     *
     ...
     */
    protected SurfaceObjectTile createTile(Sector sector, Level level, int row, int column)
    {
        return new SurfaceObjectTile(sector, level, row, column, this.cacheName);
    }

    /**
     * Returns a new tile key corresponding to the tile with the specified {@code level}, {@code row}, {@code and
     * column}. The returned key's cache name is set to {@link #cacheName}.
     *
     * @param level  The tile's Level in a {@link LevelSet}.
     * @param row    The tile's row in the Level, starting from 0 and increasing to the right.
     * @param column The tile's column in the Level, starting from 0 and increasing upward.
     ...
     * @return a tile key.
     */
    protected Object createTileKey(Level level, int row, int column)
    {
        return new TileKey(level.getLevelNumber(), row, column, this.cacheName);
    }
    ...
    protected static class SurfaceObjectTile extends TextureTile
    {
        ...
        {
            if (this.intersectingObjects == null)
                this.intersectingObjects = new ArrayList<SurfaceObject>(c);
