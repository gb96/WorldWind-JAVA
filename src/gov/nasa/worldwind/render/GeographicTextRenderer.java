...
/**
 * @author dcollins
 * @version $Id: GeographicTextRenderer.java 13710 2010-09-03 21:51:00Z tgaskins $
 */
public class GeographicTextRenderer
{
     ...
     * according to their respective priority rather then back to front, and will be drawn only if they do not
     * overlap an already drawn text. If <code>false</code> all text will be drawn back to front whether they
     * overlap or not.
     *
    ...
    /**
     * Set whether overlapping text should be culled. If <code>true</code> text items will be sorted
     * front to back according to their respective priority rather then back to front, and will be drawn only
     * if they do not overlap an already drawn text. If <code>false</code> all text will be drawn back to front
    ...
    /**
     * Get the empty margin that surrounds a text item when considering it's bounding rectangle during
     * text culling - see {@link #setCullTextEnabled(boolean)}. The smaller the margin, the closer text items
     * can get before being considered as overlapping.
     *
    ...
    /**
     * Set the empty margin that surrounds a text item when considering it's bounding rectangle during
     * text culling - see {@link #setCullTextEnabled(boolean)}. The smaller the margin, the closer text items
     * can get before being considered as overlapping.
     *
    ...
    /**
     * Get the effect used to decorate the text. Can be one of {@link AVKey#TEXT_EFFECT_SHADOW} (default),
     * {@link AVKey#TEXT_EFFECT_OUTLINE} or {@link AVKey#TEXT_EFFECT_NONE}.
     *
    ...
    /**
     * Set the effect used to decorate the text. Can be one of {@link AVKey#TEXT_EFFECT_SHADOW} (default),
     * {@link AVKey#TEXT_EFFECT_OUTLINE} or {@link AVKey#TEXT_EFFECT_NONE}.
     *
    ...
    /**
     * Get the minimum scale that can be applied to a text item when it gets farther away from the eye
     * than the view lookat point - screen center.
     *
    ...
    /**
     * Set the minimum scale that can be applied to a text item when it gets farther away from the eye
     * than the view lookat point - screen center. Use a value less then one to have the text 'fade' away.
     * A value of one will have no effect.
     *
    ...
    /**
     * Get the maximum scale that can be applied to a text item when it gets closer to the eye than the
     * view lookat point - screen center.
     *
    ...
    /**
     * Set the maximum scale that can be applied to a text item when it gets closer to the eye than the
     * view lookat point - screen center. Use a value greater then one to have the text magnified in the
     * foreground. A value of one will have no effect.
     *
    ...
    /**
     * Get the minimum opacity that can be applied to a text item when it gets farther away from the eye
     * than the view lookat point - screen center.
     *
    ...
    /**
     * Set the minimum opacity that can be applied to a text item when it gets farther away from the eye
     * than the view lookat point - screen center. Use a value less then one to have the text 'fade' away.
     * A value of one will have no effect.
     *
    ...
    private void drawMany(DrawContext dc, Iterable<GeographicText> textIterable)
    {
        ...
        double horizon = dc.getView().getFarClipDistance();
            ...
            try
            {
                if (cullText)
                {
                    ...
                    for (OrderedText ot : textList)
                    {
                        ...
                        Rectangle2D newBounds = GeographicTextRenderer.this.computeTextBounds(dc, ot, scaleAndOpacity[0]);
                        if (newBounds == null)
                            continue;
