 ...
 *
 * @author tag
 * @version $Id: PointPlacemark.java 14190 2010-12-04 01:14:26Z pabercrombie $
 */
public class PointPlacemark extends WWObjectImpl implements OrderedRenderable, Locatable, Highlightable, Exportable
{
    ...
    protected void makeOrderedRenderable(DrawContext dc)
    {
        ...
        if (this.imageIntersectsFrustum(dc) || this.isDrawLine(dc))
     ...
     */
    protected boolean imageIntersectsFrustum(DrawContext dc)
    {
        ...
        if (dc.isPickingMode())
            ...
            return dc.getView().getViewport().intersects(rect);
    }
    ...
    /**
     * Draws the placemark image as an ordered renderable.
     *
     * @param dc the current draw context.
     */
    protected void doDrawOrderedRenderable(DrawContext dc)
    {
        ...
        {
            if (this.getActiveAttributes().isUsePointAsDefaultImage())
                this.drawPoint(dc);
            return;
        }
...
//            dc.drawUnitQuadOutline(); // for debugging label placement
    ...
    /**
     * Create a {@link PickedObject} for this placemark. The PickedObject returned by this method
     * will be added to the pick list to represent the current placemark.
     *
    ...
    /**
     * Determines whether the placemark's option line should be drawn and whether it intersects the view frustum.
     *
     * @param dc the current draw context.
     *
    ...
    {
        if (!dc.isPickingMode()) // picking uses the pick color of the overall shape
        {
    ...
    protected WWTexture chooseTexture(PointPlacemarkAttributes attrs)
    {
        if (!WWUtil.isEmpty(attrs.getImageAddress()))
        {
            ...
            if (localUrl != null)
            {
                texture = new BasicWWTexture(localUrl);
    ...
    protected Rectangle computeImageRectangle(DrawContext dc)
    {
        ...
        double x = this.screenPoint.x + this.dx;
        double y = this.screenPoint.y + this.dy;
