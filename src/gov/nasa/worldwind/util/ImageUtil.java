...
/**
 * @author tag
 * @version $Id: ImageUtil.java 14142 2010-11-22 19:24:03Z garakl $
 */
...
//        return gc.createCompatibleImage(width, height, transparency);
    ...
    public static BufferedImage bucketFill(BufferedImage sourceImage)
    {
...
//        BufferedImage flipImg = new BufferedImage(w, h, img.getColorModel().getTransparency() );
    ...
    public static ByteBuffer asJPEG(DataRaster raster)
    {
        ...
        {
            image = ImageUtil.visualize((ByteBufferRaster) raster);
        }
        else
        {
    ...
    public static BufferedImage visualize(ByteBufferRaster raster)
    {
        if (null == raster)
        {
        ...
        double missingDataSignal = (double) Short.MIN_VALUE;
        double EARTH_ELEVATION_MIN = -11000d;
        double EARTH_ELEVATION_MAX = 8500d;

        if (raster.hasKey(AVKey.MISSING_DATA_SIGNAL))
            missingDataSignal = (Double) raster.getValue(AVKey.MISSING_DATA_SIGNAL);

        double missingDataReplacement = raster.hasKey(AVKey.MISSING_DATA_REPLACEMENT)
            ? (Double) raster.getValue(AVKey.MISSING_DATA_REPLACEMENT) : 0d;

        // we are building UINT DataBuffer, cannot use -32768
        missingDataReplacement = (missingDataReplacement < 0d) ? 0d : missingDataReplacement;
        ...
        double min = (null != minElevation && minElevation >= EARTH_ELEVATION_MIN) ? minElevation : 0d;
        double max = (null != maxElevation && minElevation <= EARTH_ELEVATION_MAX) ? maxElevation : EARTH_ELEVATION_MAX;
        ...
        for (int y = 0; y < height; y++)
        {
            for (int x = 0; x < width; x++)
            {
                ...
                {
                    data[BAND_Y][i] = (short) (0xFFFF & (int) (1 + ((v - min) * norm)));
