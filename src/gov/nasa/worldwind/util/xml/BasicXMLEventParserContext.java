...
public class BasicXMLEventParserContext extends AVListImpl implements XMLEventParserContext
{
    ...
    /** Contsruct an instance. Invokes {@link #initializeParsers()} and {@link #initialize()}. */
    ...
    /**
     * Contsruct an instance for a specified event reader. Invokes {@link #initializeParsers()} and {@link
     * #initialize()}.
     *
    ...
    /**
     * Contsruct an instance for a specified event reader and default namespace. Invokes {@link #initializeParsers()}
     * and {@link #initialize()}.
     *
     * @param eventReader      the event reader to use for XML parsing.
        ...
        this.addPropertyChangeListener(new PropertyChangeListener()
        {
            public void propertyChange(PropertyChangeEvent propEvent)
            {
                ...
                if (notification.getPropertyName().equals(XMLParserNotification.EXCEPTION))
                    Logging.logger().log(Level.WARNING, msg, (Exception) notification.getNewValue());
                else if (notification.getPropertyName().equals(XMLParserNotification.UNRECOGNIZED))
                    Logging.logger().log(Level.WARNING, msg, notification.getNewValue());
            }
        });
    }

    /**
     * Intializes the parser table with the default parsers for the strings, integers, etc., qualified for the default
     * namespace.
     */
