 ...
 * <b>Note:</b> In order to acheive consistent results across all platforms, it is essential to clear the texture's
 ...
 * <p/>
 * The common usage patter for OGLRenderToTextureSupport is as follows: <br/><code> DrawContext dc = ...; // Typically
 * passed in as an argument to the containing method.<br/> Texture texture = TextureIO.newTexture(new
 * TextureData(...);<br/> <br/> // Setup the drawing rectangle to match the texure dimensions, and originate from the
 ...
 * texture contents with transparent black.<br/> rttSupport.clearColor(dc, new Color(0, 0, 0, 0));<br/> // Invoke
 * desired GL rendering commands.<br/> }<br/> finally<br/> {<br/> rttSupport.endRendering(dc);<br/> }<br/> </code>
 *
 * @author dcollins
 * @version $Id: OGLRenderToTextureSupport.java 13645 2010-08-19 19:51:57Z dcollins $
 */
public class OGLRenderToTextureSupport
{
    ...
    /**
     * Speifies if framebuffer objects should be used if they are available in the current GL runtime.
     *
    ...
    protected void updateMipmaps(DrawContext dc, Texture texture)
    {
        ...
        ogsh.pushAttrib(dc.getGL(), GL2.GL_TEXTURE_BIT); // For texture binding and texture enable.
        try
        {
            texture.enable();
            texture.bind();
...
// FIXME GL_FRAMEBUFFER_INCOMPLETE_DUPLICATE_ATTACHMENT_EXT not defined
