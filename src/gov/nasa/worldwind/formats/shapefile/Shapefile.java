 ...
 * @version $Id: Shapefile.java 13937 2010-10-05 23:49:13Z dcollins $
 */
public class Shapefile extends AVListImpl implements Closeable
{
        ...
        try
        {
            ...
            if (fileCode != FILE_CODE)
            {
                ...
                throw new WWRuntimeException(
                    Logging.getMessage("SHP.NotAShapeFile", this.getValue(AVKey.DISPLAY_NAME)));
            }
     ...
     * represents the byte offset of the i'th record from the start of the Shapefile. This returns null if the JVM
     * cannot allocate enough memory to hold the index, or if the Shapefile has no accompanying index.
     *
     * @return the Shapefile's record offset index, or null if the Shapefile has no index or the index cannot be
     *         allocated.
     *
     ...
     * For details, see the OGC Coordinate Transform Service (CT) specification at <a
     * href="http://www.opengeospatial.org/standards/ct">http://www.opengeospatial.org/standards/ct</a>.
     *
     * @return coordinate system parameters parsed from the projection file.
     *
    ...
    protected ShapefileRecord readRecordFromBuffer(ByteBuffer buffer)
    {
        ...
        if (record != null)
        {
            ...
            {
                record.attributes = this.attributeFile.nextRecord();
            }
        }
    ...
    /**
     * Returns a new {@link gov.nasa.worldwind.formats.shapefile.ShapefileRecord} from the specified buffer. Returns
     * <code>null</code> if the record shape type is {@link Shapefile#SHAPE_NULL}.
     * <p/>
     * The buffer's current position is assumed to be set at the start of the record and will be set to the start of the
     * next record after this method has completed.
     *
     * @param buffer the buffer containing the record's content.
     *
     * @return a new {@link gov.nasa.worldwind.formats.shapefile.ShapefileRecord} instance.
     */
